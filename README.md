...
як ми бачимо в MCU MICR задався як 021.
...
![LinkLExample.jpg](LinkLExample.jpg)
...
* oey – мікрокоманда видачі результату Y з АЛБ на локальну шину.
...
Підключення двадцятирозрядного регістру адреси РАД до шістнадцятирозрядної ЛШ описується директивою. EWL записує в молоджу частину РАД, EWH записує в старшу частину РАД:
```
...
| `CJP <У> <А>`  | Умовний перехід по зазначеній адресі (мітки)                               |
| `CJA <У> <А>`  | Умовний перехід до мікропідпрограми по зазначеній адресі (мітки)                               |
| `CRTN <У>` | Умовне повернення з мікропідпрограми|

* JZ - це перехід до першої мікрокоманди.
* JMAP - це перехід за адресою локальної шини.
* CONT - це перехід до наступної мікрокоманди.
* CJP <Умова> <Адреса> - це перехід при умові. Якщо умова дорівнює 0, то перехід не відбувається і програма іде до наступної мікрокоманди, в іншому випадку переходить за цією адресою.


Приклад по Колоку (Перший спосіб множення)
``` 
link l1:ct
...
{or r13, r13, r6;}
``` 
Для початку задаємо значення X, Y, Z та CT. Через macro розписуємо мікрокоманди перевірки знаку, модуль числа, перенесення числа в інший регістр, додавання та віднімання. Потім розписуємо знаки X, Y, Z. Для XY ми робимо перевірку знака,і якщо число від'ємне - то беремо його в модуль і переходимо далі. Потім іде пересування в наступну мітку при додатньому числі. Для Z ми переносимо число з регістра Х (r4) в  r6 та перевіряємо запис знака результата. Для m1 та m2 ми повторюємо дії як при мікроалгоритмі (Зсув праворуч на одиницю, віднімання від CT і так далі). Останні команди ми переносимо в регістри фінальний результат.
